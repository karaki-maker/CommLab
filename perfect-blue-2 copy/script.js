...
document.querySelectorAll('.deleted-item').forEach(item => {
  item.addEventListener('click', () => {
    ...
    // if it's the red dress, ramp the horror + show popup
...
function addNewMeManiaEmail() {
  ...
  // auto-open inbox + this email
  openWindow('inbox-window');
  displayEmail('m5');
}
...
// YES buttons -> ending.html
document.querySelectorAll('.identity-yes').forEach(btn => {
